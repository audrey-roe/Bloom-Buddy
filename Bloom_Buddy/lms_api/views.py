...
def webadmin(request):
    ...
    orders = OrderSerializer(Order.objects.all(), many=True)                                         # Create an order serializer and apply
...
@api_view(['GET'])
def alltime(request):
    f = timingserial(timing.objects.all(), many=True)                                                # Create a time serializers
    context = {'f':f}
    return Response(context,status=HTTP_200_OK)
